    ...
    function copydirs($source, $dest)
    {
        ...
        foreach ($iterator =
            new RecursiveIteratorIterator(new RecursiveDirectoryIterator($source,
                RecursiveDirectoryIterator::SKIP_DOTS), RecursiveIteratorIterator::SELF_FIRST) as $item) {
            if ($item->isDir()) {
...
if (! function_exists('addtion')) {
    ...
    function addtion($items, $fields)
    {
        ...
        foreach ($fieldsArr as $k => &$v) {
            ...
            $v['display'] =
                isset($v['display']) ? $v['display'] : str_replace(['_ids', '_id'], ['_names', '_name'], $v['field']);
        ...
        foreach ($fieldsArr as $k => $v) {
            ...
            $result[$v['field']] =
                $model->where($primary, 'in', $ids[$v['field']])->column("{$primary},{$v['column']}");
        }
...
if (! function_exists('letter_avatar')) {
    ...
    function letter_avatar($text)
    {
        ...
        $src =
            base64_encode('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="100" width="100"><rect fill="' .
                          $bg .
...
if (! function_exists('check_cors_request')) {
    ...
    function check_cors_request()
    {
        if (isset($_SERVER['HTTP_ORIGIN']) && $_SERVER['HTTP_ORIGIN']) {
            ...
            if (in_array("*", $domainArr) ||
                in_array($_SERVER['HTTP_ORIGIN'], $domainArr) ||
                (isset($info['host']) && in_array($info['host'], $domainArr))
            ) {
...
if (! function_exists('kuaibao_account')) {
    function kuaibao_account(\app\common\model\User $user, \app\admin\model\Order $order)
    {
